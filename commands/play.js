...
module.exports = {
    name: "play",
    description: "Phát nhạc hoặc danh sách phát.",
    ...
    run: async (client, message, args, { GuildDB }) => {
        if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ | **Bạn phải ở trong kênh thoại để mở nhạc.**");
        if (message.guild.me.voice.channel && message.member.voice.channel.id !== message.guild.me.voice.channel.id) return client.sendTime(message.channel, "❌ | **Bạn phải ở trong kênh thoại để dùng lệnh.**");
        let SearchString = args.join(" ");
        if (!SearchString) return client.sendTime(message.channel, `**Đã dùng - **\`${GuildDB.prefix}play [query]\``);
       ...
       return client.sendTime(message.channel,"❌ | **Lavalink không thể kết nối**");
        }
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`Đã thêm danh sách phát vào hàng đợi`, message.author.displayAvatarURL());
                    ...
                    return client.sendTime(message.channel, "**Không tìm thấy **" + SearchString);
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, "❌ | **Không có bài nào đang phát cả.**");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**Không có kết quả cho - **" + SearchString);
                else if (Searched.loadType == "PLAYLIST_LOADED") {
                    player.queue.add(Searched.tracks);
                    if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
                    SongAddedEmbed.setAuthor(`Danh sách phát đã thêm vào hàng đợi `, client.config.IconURL);
