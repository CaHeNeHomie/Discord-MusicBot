...
module.exports = {
    name: "seek",
    description: "Chuyển đến vị trí khác tong bài hát.",
    usage: "[(time) h m s]",
    permissions: {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!player.queue.current.isSeekable) return client.sendTime(message.channel, "❌ | **Tôi không thể bạn ơi !**");
        let SeekTo = client.ParseHumanTime(args.join(" "));
        if (!SeekTo) return client.sendTime(message.channel, `**Đã dùng - **\`${GuildDB.prefix}seek [(time) h m s]\` \n**Ví dụ - **\`${GuildDB.prefix}seek 2m 10s\``);
